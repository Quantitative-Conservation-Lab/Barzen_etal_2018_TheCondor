...
#data
dataset.1 <- list(JDatecorr=JDatecorr,year=year,exper.Avg=exper.Avg,pair=pair,n=n,n.pair=n.pair,n.year=n.year,year.org=year.org)

#initial values
inits.1 <- function(){
  list (int.pair=runif(1),sigma.pair=runif(1),w=rbinom(2,1,0.5),b.exper.Avg=runif(1,-1,1))
}

#parameters
parameters.1 <- c("tau","int.pair","sigma.pair","sigma.year","b.year","b.exper.Avg","w")
...
cat("
    ...
    mu[i] <- b1[pair[i]] + b2[year.org[i]] + w[1]*b.year*year[i] + w[2]*b.exper.Avg*exper.Avg[i]
    }
