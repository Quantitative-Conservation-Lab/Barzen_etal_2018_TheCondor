...
par(mar=c(5,5,4,2))
plot(x=c(2004.9:2012.9),y=count.inexper.yr,type='h',col='gray70',xlim=c(2005,2013.1),ylim=c(0,20),xlab="Year",ylab="Number of Pairs",lwd=5,cex.axis=2,cex.lab=2)
lines(x=c(2005.1:2013.1),y=count.exper.yr,type='h',col='black',lwd=5)
lines(x=c(2005:2013),y= count.midexper.yr,type='h',col='gray35',lwd=5)
...
#data
dataset.1 <- list(JDatecorr=JDatecorr,exper.M=exper.M,exper.F=exper.F,exper.Avg=exper.Avg,CumAtt.M=CumAtt.M,CumAtt.F=CumAtt.F,CumAtt.Avg=CumAtt.Avg,pair=pair,n=n,n.pair=n.pair,n.year=n.year,year=year.org)
...
#parameters
parameters.1 <- c("tau","int.pair","sigma.pair","sigma.year","b.eM","b.eF","b.eAvg","b.cM","b.cF","b.cAvg","w")
...
cat("
    ...
    mu[i] <- b1[pair[i]] + b2[year[i]] + w[1]*b.eF*exper.F[i] + w[2]*b.eM*exper.M[i] + w[3]*b.eAvg*exper.Avg[i] + w[4]*b.cF*CumAtt.F[i] + w[5]*b.cM*CumAtt.M[i] + w[6]*b.cAvg*CumAtt.Avg[i]
    }
    ...
    b.eM ~ dnorm(0,tau.model)
    b.eF ~ dnorm(0,tau.model)
    b.eAvg ~ dnorm(0,tau.model)
    b.cM ~ dnorm(0,tau.model)
    b.cF ~ dnorm(0,tau.model)
    b.cAvg ~ dnorm(0,tau.model)
...
#run MCMC
library("jagsUI")
jagsfit.1 <- jags(data=dataset.1, inits=inits.1, parameters.to.save=parameters.1, n.chains=3, n.burnin = 20000, n.iter = 60000, n.thin=1, model.file="model.1.txt",parallel=TRUE)
